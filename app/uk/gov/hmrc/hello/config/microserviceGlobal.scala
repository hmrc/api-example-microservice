...
import uk.gov.hmrc.hello.controllers.{ErrorConversion, ErrorGenericBadRequest, ErrorInternalServerError, ErrorNotFound, ErrorUnauthorized}
...
object MicroserviceGlobal extends DefaultMicroserviceGlobal with RunMode with ServiceLocatorRegistration with ErrorConversion {
  override val auditConnector = MicroserviceAuditConnector
  ...
  override def onError(request: RequestHeader, ex: Throwable): Future[Result] = {
    ...
    super.onError(request, ex).map[Result] { res =>
      res.header.status match {
        case 401 => ErrorUnauthorized
  ...
  override def onBadRequest(request: RequestHeader, error: String): Future[Result] = {
    implicit val r = request
    Future.successful(ErrorGenericBadRequest)
  }
